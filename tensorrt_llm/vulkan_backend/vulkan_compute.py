...
class _VulkanCompute:
    ...
    def _init_funcs(self):
        ...
        # tllm_vulkan_layer_norm(void* input, void* gamma, void* beta, void* output,
        #                        float eps, size_t hiddenDim, size_t tokenCount)
        if hasattr(lib, "tllm_vulkan_layer_norm"):
            lib.tllm_vulkan_layer_norm.argtypes = [
                ctypes.c_void_p, ctypes.c_void_p, ctypes.c_void_p, ctypes.c_void_p,
                ctypes.c_float, ctypes.c_size_t, ctypes.c_size_t,
            ]
            lib.tllm_vulkan_layer_norm.restype = ctypes.c_int32
        ...
        # tllm_vulkan_sigmoid_mul(void* a, void* b, void* output, size_t elementCount)
        lib.tllm_vulkan_sigmoid_mul.argtypes = [
            ctypes.c_void_p, ctypes.c_void_p, ctypes.c_void_p, ctypes.c_size_t,
        ]
        lib.tllm_vulkan_sigmoid_mul.restype = ctypes.c_int32

        # tllm_vulkan_elementwise_mul(void* a, void* b, void* output, size_t elementCount)
        lib.tllm_vulkan_elementwise_mul.argtypes = [
            ctypes.c_void_p, ctypes.c_void_p, ctypes.c_void_p, ctypes.c_size_t,
        ]
        lib.tllm_vulkan_elementwise_mul.restype = ctypes.c_int32

        # tllm_vulkan_scale_rows(void* input, void* scale, void* output,
        #                        uint32_t rows, uint32_t cols)
        lib.tllm_vulkan_scale_rows.argtypes = [
            ctypes.c_void_p, ctypes.c_void_p, ctypes.c_void_p,
            ctypes.c_uint32, ctypes.c_uint32,
        ]
        lib.tllm_vulkan_scale_rows.restype = ctypes.c_int32

        # tllm_vulkan_cast(void* input, void* output, size_t elementCount, int32_t targetDtype)
        lib.tllm_vulkan_cast.argtypes = [
            ctypes.c_void_p, ctypes.c_void_p, ctypes.c_size_t, ctypes.c_int32,
        ]
        lib.tllm_vulkan_cast.restype = ctypes.c_int32

        # tllm_vulkan_index_add(void* output, void* indices, void* values,
        #                       uint32_t outputRows, uint32_t valueRows, uint32_t cols)
        lib.tllm_vulkan_index_add.argtypes = [
            ctypes.c_void_p, ctypes.c_void_p, ctypes.c_void_p,
            ctypes.c_uint32, ctypes.c_uint32, ctypes.c_uint32,
        ]
        lib.tllm_vulkan_index_add.restype = ctypes.c_int32

        # tllm_vulkan_gather(src, indices, output, numIndices)
        lib.tllm_vulkan_gather.argtypes = [
            ctypes.c_void_p, ctypes.c_void_p, ctypes.c_void_p,
            ctypes.c_uint32,
        ]
        lib.tllm_vulkan_gather.restype = ctypes.c_int32

        # tllm_vulkan_fill(output, value, elementCount)
        lib.tllm_vulkan_fill.argtypes = [
            ctypes.c_void_p, ctypes.c_float, ctypes.c_size_t,
        ]
        lib.tllm_vulkan_fill.restype = ctypes.c_int32

        # tllm_vulkan_compare_eq(input, output, threshold, elementCount)
        lib.tllm_vulkan_compare_eq.argtypes = [
            ctypes.c_void_p, ctypes.c_void_p, ctypes.c_float, ctypes.c_size_t,
        ]
        lib.tllm_vulkan_compare_eq.restype = ctypes.c_int32
    ...
    def index_add(self, output_ptr, indices_ptr, values_ptr, output_rows, value_rows, cols):
        if not hasattr(self._funcs, "tllm_vulkan_index_add"):
            raise RuntimeError("tllm_vulkan_index_add not available in this DLL build")
        return self._funcs.tllm_vulkan_index_add(output_ptr, indices_ptr, values_ptr,
                                                  output_rows, value_rows, cols)

    def gather(self, src_ptr, indices_ptr, output_ptr, num_indices):
        if not hasattr(self._funcs, "tllm_vulkan_gather"):
            raise RuntimeError("tllm_vulkan_gather not available in this DLL build")
        return self._funcs.tllm_vulkan_gather(src_ptr, indices_ptr, output_ptr, num_indices)

    def fill(self, output_ptr, value, element_count):
        if not hasattr(self._funcs, "tllm_vulkan_fill"):
            raise RuntimeError("tllm_vulkan_fill not available in this DLL build")
        return self._funcs.tllm_vulkan_fill(output_ptr, value, element_count)

    def compare_eq(self, input_ptr, output_ptr, threshold, element_count):
        return self._funcs.tllm_vulkan_compare_eq(input_ptr, output_ptr, threshold, element_count)
    ...
    def layer_norm(self, input, gamma, beta, output, eps, hidden_dim, token_count):
        return self._funcs.tllm_vulkan_layer_norm(input, gamma, beta, output, eps, hidden_dim, token_count)
    ...
    def sigmoid_mul(self, a, b, output, element_count):
        return self._funcs.tllm_vulkan_sigmoid_mul(a, b, output, element_count)

    def elementwise_mul(self, a, b, output, element_count):
        return self._funcs.tllm_vulkan_elementwise_mul(a, b, output, element_count)

    def scale_rows(self, input_ptr, scale_ptr, output_ptr, rows, cols):
        return self._funcs.tllm_vulkan_scale_rows(input_ptr, scale_ptr, output_ptr, rows, cols)

    def cast(self, input_ptr, output_ptr, element_count, target_dtype):
        return self._funcs.tllm_vulkan_cast(input_ptr, output_ptr, element_count, target_dtype)
    ...
    def append_paged_kv_cache(self, *args):
        if not hasattr(self._funcs, "tllm_vulkan_append_paged_kv_cache"):
            raise RuntimeError("tllm_vulkan_append_paged_kv_cache not available in this DLL build")
...
tllm_vulkan_layer_norm = _vk.layer_norm if is_available() else None
...
tllm_vulkan_sigmoid_mul = _vk.sigmoid_mul if is_available() else None
tllm_vulkan_elementwise_mul = _vk.elementwise_mul if is_available() else None
tllm_vulkan_scale_rows = _vk.scale_rows if is_available() else None
tllm_vulkan_cast = _vk.cast if is_available() else None
tllm_vulkan_index_add = _vk.index_add if is_available() else None
tllm_vulkan_gather = _vk.gather if is_available() else None
tllm_vulkan_fill = _vk.fill if is_available() else None
tllm_vulkan_compare_eq = _vk.compare_eq if is_available() else None
